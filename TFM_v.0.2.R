...
fdi <- fdi_raw %>%
  ...
  select(country_name, country_code, as.character(YEAR_START:YEAR_END)) %>%
...
cat("FDI loaded:", n_distinct(fdi$country_code), "countries |",
...
inflation  <- load_wb("Inf_CP.xls",    "inflation")
gdp_growth <- load_wb("GDP_Growth.xls","gdp_growth")
trade_open <- load_wb("Trade_GDP.xls", "trade_open")
...
raw_bil <- as.data.frame(
  read_excel("Bilat_EUR.xlsx", sheet = "Sheet 1",
...
er_long <- er %>%
  select(-currency) %>%
  pivot_longer(-country_code, names_to = "month", values_to = "eur_rate") %>%
  mutate(month    = as.Date(paste0(month, "-01")),
         year     = as.integer(format(month, "%Y")),
...
raw_neer <- as.data.frame(
  read_excel("NEER.xlsx", sheet = "Sheet 1",
             col_names = FALSE, skip = 8)
)
...
neer <- neer_raw %>%
  ...
  pivot_longer(-country_code, names_to = "month", values_to = "neer") %>%
  mutate(month = as.Date(paste0(month, "-01")),
         year  = as.integer(format(month, "%Y")),
...
cat("NEER loaded:", n_distinct(neer$country_code), "countries |",
...
panel <- fdi %>%
  mutate(
    ...
    group       = ifelse(country_code %in% eurozone, "Eurozone", "Non-euro EU"),
    period      = case_when(
      year <= 2007 ~ "Pre-crisis (2000-2007)",
      year <= 2012 ~ "Crisis (2008-2012)",
      TRUE         ~ "Post-crisis (2013-2022)"
    ),
    period = factor(period, levels = c("Pre-crisis (2000-2007)",
                                       "Crisis (2008-2012)",
...
cat("\nPanel:", nrow(panel), "rows |", n_distinct(panel$country_code), "countries\n")
cat("Variables:", names(panel), "\n")
...
cat("\n--- FDI summary by group (mean, sd, min, max) ---\n")
panel %>%
  group_by(group) %>%
  summarise(
    n      = sum(!is.na(fdi)),
...
p1 <- panel %>%
  ...
  annotate("text", x = 2008.5, y = 10, label = "GFC",
...
p2 <- panel %>%
  ...
  summarise(mean_fdi = mean(fdi, na.rm = TRUE), .groups = "drop") %>%
  ggplot(aes(x = reorder(country_code, mean_fdi), y = mean_fdi, fill = group)) +
  geom_col() +
  scale_fill_manual(values = c("Eurozone" = col_euro,
                               "Non-euro EU" = col_noneuro), name = NULL) +
  coord_flip() +
...
# ================================================================
# We use lm() with country and year dummies to control for fixed effects.
# This replicates the "within" (fixed-effects) panel estimator using OLS.
#
# Model:
...
cat("\n--- Equation 1: current ERV (contemporaneous) ---\n")
